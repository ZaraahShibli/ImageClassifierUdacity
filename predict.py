...
ap.add_argument('input', default='./flowers/test/10/image_07104.jpg', nargs='?', action="store", type = str)
...
def main():
    model=unit.load_checkpoint(path)
    with open('cat_to_name.json', 'r') as json_file:
        cat_to_name = json.load(json_file)
    print(path_image)
    probabilities = unit.predict(path_image, model, number_of_outputs)
